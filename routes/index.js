  ...
  for (var i = 0; i < sentenceArray.length; i++) {
    ...
    if (anagrams.indexOf(sentenceArray[i]) === -1) {
      ...
      if (CheckIfAnagram(word, sentenceArray[i])) {
        ...
        // add the anagram to the array and remove all punctuation from the word and convert it to lowercase
...
router.post('/:endpoint', function (req, res) {
  ...
  //get the strings from the request body
  ...
  // check which endpoint is being called and call the appropriate function
  if (req.params.endpoint === 'A') {

    outcome = CheckIfAnagram(string1, string2);

  } else if (req.params.endpoint === 'B') {

    outcome = FindAnagrams(string1, string2);

  } else if (req.params.endpoint === 'C') {
  
    outcome = FindAnagramGroups(string1);

  } else if (req.params.endpoint === 'C2') {

    outcome = FindAnagramGroups2(string1);

  } else {

    outcome = "Invalid endpoint!";
  }

  // build the json
  let outcome_json = { 'outcome': outcome };

  // send the json to the client with status code 200
